...
def inject_css():
    ...
    * { font-family: var(--mono); letter-spacing: 0; }
    ...
    .sidebar-stamp strong { color: var(--muted); font-weight: 400; }
    .sidebar-icons {
        display: flex;
        flex-direction: column;
        align-items: center;
    ...
    .sidebar-icon:hover { color: var(--teal); }
    .sidebar-icon svg { width: 16px; height: 16px; stroke: currentColor; fill: none; stroke-width: 1.5; stroke-linecap: round; stroke-linejoin: round; }
    .sidebar-tooltip {
        position: absolute;
        left: 38px;
    ...
    .sidebar-icon-wrap:hover .sidebar-tooltip { opacity: 1; }
    ...
    .masthead-left strong { color: var(--ink); font-weight: 500; }
    .masthead-right {
        font-size: 10px;
    ...
    .case-file:hover .top-bar, .case-file.selected .top-bar { height: 3px; }
    ...
    .case-status.open { color: var(--teal); border-color: rgba(78,203,160,0.25); }
    ...
    .case-file.featured .case-signal { font-size: 12px; }
    ...
    .case-file:hover .chip, .case-file.selected .chip {
        color: var(--muted);
        border-color: var(--border2);
    }
    ...
    .case-file:hover .case-arrow, .case-file.selected .case-arrow {
        color: var(--teal);
    ...
    .detail-repo a:hover { border-color: rgba(123,159,212,0.6); color: var(--indigo); }
    ...
    .footer-left strong { color: var(--muted); font-weight: 400; }
    .footer-right {
        font-size: 10px;
        letter-spacing: 0.1em;
        color: var(--dim);
...
def case_file_html(case, featured=False, selected=False):
    ...
    chips = "".join(f"<span class='chip'>{s}</span>" for s in case["stack"][:5])
    signal = case["signal"] if featured else ""
    signal_html = f"<div class='case-signal'>{signal}</div>" if signal else ""

    return f"""
    <div class="case-file {featured_cls} {selected_cls}">
        <div class="top-bar" style="background:{case['accent']}"></div>
        <div class="case-id-row">
            <div class="case-id-left">
                <div class="dot" style="background:{case['accent']}"></div>
                CASE {case['id']} &nbsp;/&nbsp; {case['domain']}
            </div>
            <div class="case-status {status_cls}">{status_txt}</div>
        </div>
        <div class="case-verdict">{case['verdict']}</div>
        {signal_html}
        <div class="chip-row">{chips}</div>
        <div class="case-footer">
            <span class="case-role">{case['name']} &nbsp;·&nbsp; {case['role']}</span>
...
def detail_panel_html(case):
    ...
    <div class="detail-panel">
        <div class="top-bar" style="background: linear-gradient(90deg, {case['accent']}, transparent)"></div>
        <div class="detail-inner">
            <div class="detail-header">
                <div>
                    <div class="detail-name">{case['name']}</div>
                    <div class="detail-meta">CASE {case['id']} &nbsp;·&nbsp; {case['domain']} &nbsp;·&nbsp; {case['role']}</div>
                </div>
                <div class="detail-repo">
                    <a href="{case['repo']}" target="_blank">
            ...
            <div class="detail-grid">
                <div class="detail-block">
                    ...
                    <div class="detail-text">{case['problem']}</div>
                </div>
                <div class="detail-block">
                    <div class="detail-label">Engineering decision</div>
                    <div class="detail-text">{case['decision']}</div>
                </div>
                <div class="detail-block">
                    <div class="detail-label">Core technical challenge</div>
                    <div class="detail-text">{case['challenge']}</div>
                </div>
                <div class="detail-block">
                    <div class="detail-label">What I would do differently</div>
                    <div class="detail-text">{case['differently']}</div>
                </div>
            </div>
        </div>
    </div>
...
def notes_row():
    ...
    for note in NOTES:
        notes_html += f"""
        <div class="note-file">
            <div class="note-label">signal &nbsp;/&nbsp; {note['label']}</div>
...
def main():
    ...
    # ── case grid ──
    # Row 1: featured (col 1, spans rows) + case 2 top-right + case 3 top-right
    # Row 2: featured continues + case 4 bottom-right + case 5 bottom-right
    # We render via a 3-column streamlit layout

    col_feat, col_mid, col_right = st.columns([2, 1, 1], gap="small")

    with col_feat:
        c = CASES[0]
        selected = st.session_state.selected == c["id"]
        st.markdown(case_file_html(c, featured=True, selected=selected), unsafe_allow_html=True)
        if st.button("", key=f"btn_{c['id']}", help=c['name']):
            st.session_state.selected = None if selected else c["id"]
            st.rerun()

    with col_mid:
        for c in [CASES[1], CASES[3]]:
            selected = st.session_state.selected == c["id"]
            st.markdown(case_file_html(c, selected=selected), unsafe_allow_html=True)
            if st.button("", key=f"btn_{c['id']}", help=c['name']):
                st.session_state.selected = None if selected else c["id"]
                st.rerun()
    ...
    if st.session_state.selected:
        active = next((c for c in CASES if c["id"] == st.session_state.selected), None)
        if active:
